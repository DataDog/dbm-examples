...
class AuroraPostgresAutoDiscoverStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, env: Environment, **kwargs) -> None:
        ...
        # add tags to aurora cluster for datadog autodiscovery
        Tags.of(aurora_cluster).add("datadoghq.com/scrape", "true")
            ...
            iam.Policy(
                ...
                statements=[
                    ...
                    iam.PolicyStatement(
                        ...
                        resources=[
                            f"arn:aws:rds:{self.region}:{self.account}:cluster:*",  # allow dd-agent to scrape aurora cluster
                            f"arn:aws:rds:{self.region}:{self.account}:db:*"  # allow dd-agent to list db instances
                        ],
                        effect=iam.Effect.ALLOW,
                    ),
                ],
        ...
        dd_agent.user_data.add_commands(
            f'DD_API_KEY={dd_api_key} DD_SITE="datadoghq.com" DD_AGENT_DIST_CHANNEL=beta DD_AGENT_MAJOR_VERSION=7 DD_AGENT_MINOR_VERSION=52.0~dbm~aurora~autodiscovery~beta~0.3-1 bash -c "$(curl -L https://s3.amazonaws.com/dd-agent/scripts/install_script_agent7.sh)"'
        )

        # Update datadog agent config
        with open(os.path.join(os.path.dirname(__file__), "datadog.yaml")) as f:
            config = f.read()
            # append autodiscovery config to /etc/datadog-agent/datadog.yaml
            dd_agent.user_data.add_commands(
                *[
        ...
        with open(os.path.join(os.path.dirname(__file__), "auto_conf.yaml")) as f:
            config = f.read()
            dd_agent.user_data.add_commands(
                *[
                    "cat << EOF > /etc/datadog-agent/conf.d/postgres.d/auto_conf.yaml",
                    config,
                    "EOF",
